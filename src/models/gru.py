...
class CNNGRUHead(nn.Module):
    """Single CNN+GRU head (Dua et al. 2021)."""

    def __init__(self, n_channels: int, kernel_size: int):
        super().__init__()
        pad = kernel_size // 2
        self.conv1 = nn.Conv1d(n_channels, 64, kernel_size, padding=pad)
        self.conv2 = nn.Conv1d(64, 64, kernel_size, padding=pad)
        self.dropout = nn.Dropout(0.5)
        self.pool = nn.MaxPool1d(2)

        flat_size = 64 * 16  # 64 filters × (32 // 2) steps = 1024
        self.gru1 = nn.GRU(flat_size, 32, batch_first=True)
        self.gru2 = nn.GRU(32, 32, batch_first=True)
        self.drop2 = nn.Dropout(0.5)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (batch, n_seq, n_steps, n_channels)
        batch, n_seq, n_steps, C = x.shape

        # TimeDistributed Conv
        x = x.view(batch * n_seq, n_steps, C).permute(0, 2, 1).contiguous()
        x = F.relu(self.conv1(x))
        x = F.relu(self.conv2(x))
        x = self.dropout(x)
        x = self.pool(x)
        x = x.flatten(1)  # (batch*n_seq, 1024)
        x = x.view(batch, n_seq, -1)  # (batch, n_seq, 1024)

        x, _ = self.gru1(x)  # (batch, n_seq, 32)
        x, _ = self.gru2(x)  # (batch, n_seq, 32)
        x = x[:, -1, :]  # (batch, 32)
        return self.drop2(x)


class MultibranchCNNGRU(nn.Module):
    """
    Dua et al. (2021) architecture.
    ...
    def __init__(
        ...
        super().__init__()
        self.head3 = CNNGRUHead(n_channels, kernel_size=3)
        self.head7 = CNNGRUHead(n_channels, kernel_size=7)
        self.head11 = CNNGRUHead(n_channels, kernel_size=11)

        concat_size = 3 * 32  # 96

        self.dense = nn.Linear(concat_size, 128)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = torch.cat([h3, h7, h11], dim=1)  # (batch, 96)
...
class PyTorchGRU:
    def __init__(
        ...
        self.model = MultibranchCNNGRU(n_channels, n_classes).to(self.device)
...
def run_gru(data_dir: Path, output_dir: Path):
    ...
    save_metrics_report(all_metrics, output_dir, "CNN-GRU (Dua et al.)", timing=timing)


def main():
    processed_base = Path("PAMAP2_Dataset/processed")
    results_base = Path("results/gru")
    for subdir in ["normal", "feature_selection"]:
        run_gru(processed_base / subdir, results_base / subdir)
